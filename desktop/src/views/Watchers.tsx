...
export function Watchers({ watchers, onAdd, onUpdate, onRemove }: {
  ...
  return <div className="view">
    ...
    <section className="panel watcher-create"><div><h2>Monitor a project</h2><p>Choose a website address and the project folder to watch.</p></div><form onSubmit={submit}><input value={url} onChange={event => setUrl(event.target.value)} placeholder="Website address" aria-label="Website address"/><select value={mode} onChange={event => setMode(event.target.value as 'quick' | 'full')} aria-label="Audit type"><option value="quick">Quick audit</option><option value="full">Full audit</option></select><button className="primary-btn"><FolderOpen size={18}/> Choose folder</button></form></section>
