...
export function Dashboard({ items, localHistory, workspace, defaultMode, servers, onPunch, onOpen, onHistory }: {
  ...
  return <div className="view dashboard-view">
    ...
    {active.length > 0 && <section className="active-queue"><div><span className="pulse-dot"/><strong>{active.length} active {active.length === 1 ? 'punch' : 'punches'}</strong></div>{active.map(scan => <div className="queue-row" key={scan.id}><span>{new URL(scan.target_url).host}</span><div className="progress-track"><i/></div><small>{scan.stage}</small><button onClick={() => void window.gorillaPunch.scans.cancel(scan.id)}>Cancel</button></div>)}</section>}
    <section className="overview-grid">{stats.map(stat => <article key={stat.label}><stat.icon size={21}/><span>{stat.label}</span><strong>{stat.value}</strong></article>)}</section>
    <section className="panel recent-panel"><div className="section-title"><div><span className="eyebrow">LATEST EVIDENCE</span><h2>Recent punches</h2></div><button className="text-btn" onClick={onHistory}>View all</button></div><ScanList items={items.slice(0, 6)} onOpen={onOpen}/></section>
  </div>;
}
