...
export function ReportView({ report, source, onBack, onDelete, onSync, notify }: {
  ...
  return <div className="report-view">
    ...
    {tab === 'findings' && <section className="report-content"><div className="finding-filters"><label><MagnifyingGlass size={17}/><input aria-label="Search evidence" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search evidence…"/></label><select aria-label="Filter by severity" value={severity} onChange={event => setSeverity(event.target.value)}><option value="">All severities</option>{['BLOCKER','CRITICAL','HIGH','MEDIUM','LOW','OPPORTUNITY','PASSED'].map(value => <option key={value}>{value}</option>)}</select></div><div className="desktop-findings">{filtered.map(finding => <button key={finding.id} onClick={() => setSelected(finding)}><span className={`severity ${finding.severity}`}>{finding.severity}</span><span><strong>{finding.title}</strong><small>{categoryNames[finding.category] || finding.category}</small><p>{finding.evidence}</p></span><Eye size={18}/></button>)}</div>{!filtered.length && <div className="empty-panel"><Check size={28}/><strong>No findings match.</strong></div>}</section>}
