...
export function ReportView({ report, source, onBack, onDelete, onSync, notify }: {
  ...
  return <div className="report-view">
    ...
    {tab === 'findings' && <section className="report-content"><div className="finding-filters"><label><MagnifyingGlass size={17}/><input aria-label={t('report.searchLabel')} value={query} onChange={event => setQuery(event.target.value)} placeholder={t('report.search')}/></label><Dropdown<string> className="finding-severity-filter" ariaLabel={t('report.filterSeverity')} value={severity} onChange={setSeverity} options={[{ value: '', label: t('report.allSeverities') }, ...['BLOCKER','CRITICAL','HIGH','MEDIUM','LOW','OPPORTUNITY','PASSED'].map(value => ({ value, label: severityLabel(value) }))]}/></div><div className="desktop-findings">{filtered.map(finding => <button key={finding.id} onClick={() => setSelected(finding)}><span className={`severity ${finding.severity}`}>{severityLabel(finding.severity)}</span><span><strong>{finding.title}</strong><small>{categoryLabel(finding.category)}</small><p>{finding.evidence}</p></span><Eye size={18}/></button>)}</div>{!filtered.length && <div className="empty-panel"><Check size={28}/><strong>{t('report.noneMatch')}</strong></div>}</section>}
    ...
    {selected && <aside className="finding-drawer"><div className="drawer-head"><span className={`severity ${selected.severity}`}>{severityLabel(selected.severity)}</span><Tooltip content={t('report.closeDetails')}><button aria-label={t('report.closeDetails')} onClick={() => setSelected(null)}><X size={20}/></button></Tooltip></div><h2>{selected.title}</h2><Detail title={t('report.why')} value={selected.why_it_matters}/><Detail title={t('report.evidence')} value={selected.evidence}/><Detail title={t('report.reproduce')} value={selected.reproduction}/><Detail title={t('report.recommendation')} value={selected.recommendation}/>{(selected.element || selected.instances.find(instance => instance.element)?.element) && <button className="primary-btn inspector-btn" onClick={() => void window.gorillaPunch.reports.inspect(selected.affected_url, selected.element || selected.instances.find(instance => instance.element)?.element)}><Eye size={18}/> {t('report.inspect')}</button>}</aside>}
  </div>;
}
