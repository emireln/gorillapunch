...
export function App() {
  ...
  const openReport = async (item: ScanItem) => {
    ...
    try {
      ...
      if (!result) throw new Error('Report not found.'); setReport(result);
  ...
  const addWatcher = async (url: string, mode: 'quick' | 'full') => {
    try { const watcher = await window.gorillaPunch.watchers.add(url, mode); if (watcher) { setWatchers(current => [...current, watcher]); notify('Project watcher added.'); } }
    catch (error) { notify(errorMessage(error), 'error'); }
  };
  ...
  const deleteReport = async () => {
    ...
    try {
      ...
      setReport(null); setView('history'); notify('Report deleted.');
    } catch (error) { notify(errorMessage(error), 'error'); }
  };
