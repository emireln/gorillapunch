...
export function TitleBar({ workspace, cloud, online, onPunch, onProfile }: { workspace: WorkspaceMode; cloud: CloudState | null; online: boolean; onPunch(url: string): void; onProfile(): void }) {
  ...
  return <header className="titlebar">
    ...
    <div className="titlebar-status no-drag">{workspace === 'cloud' && cloud?.authenticated ? <Tooltip content={t('app.profile')}><button className="titlebar-avatar" type="button" aria-label={t('app.profile')} onClick={onProfile}>{cloud.avatarDataUrl ? <img src={cloud.avatarDataUrl} alt=""/> : <UserCircle size={27}/>}</button></Tooltip> : <Tooltip content={t(online ? 'app.online' : 'app.offline')}><span className={`connection-dot ${online ? 'online' : ''}`} aria-label={t(online ? 'app.online' : 'app.offline')} tabIndex={0}/></Tooltip>}</div>
