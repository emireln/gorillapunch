...
export function TitleBar({ workspace, cloud, online, onPunch }: { workspace: WorkspaceMode; cloud: CloudState | null; online: boolean; onPunch(url: string): void }) {
  ...
  return <header className="titlebar">
    ...
    <div className="titlebar-status no-drag"><span className={`connection-dot ${online ? 'online' : ''}`}/><span>{workspace === 'local' ? 'Local only' : cloud?.authenticated ? 'Cloud connected' : 'Cloud signed out'}</span></div>
