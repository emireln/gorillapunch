		...
		"void main(void){" +
			...
			"else{" +  // 3) calculate color with lights and fog
				...
				"gl_FragColor.rgb*=smoothstep(" +
					...
					");" +
				"float warm=step(.14,t.r-t.b)*step(.08,t.r-t.g);" +
				"float lum=dot(gl_FragColor.rgb,vec3(.3,.59,.11));" +
...
function renderer_init() {
	...
	ambient_uniform = gl.getUniformLocation(shader_program, "ambient");
	purple_uniform = gl.getUniformLocation(shader_program, "brandPurple");
	red_uniform = gl.getUniformLocation(shader_program, "brandRed");
...
function renderer_end_frame() {
	...
	gl.uniform3fv(red_uniform, shot_palette.red);
	
	gl.clearColor(shot_palette.background[0], shot_palette.background[1], shot_palette.background[2], 1);
